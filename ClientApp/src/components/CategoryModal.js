...
function CategoryModal({ isOpen, setIsOpen, selectedCategory, categories, setCategories }) {
    ...
    function updateCategory() {
        axios.put(`https://localhost:7165/categories/${categoryUpdate.id}`, categoryUpdate)
            .then(response => {
                let updatedCategories = [...categories] //pushing all elements into array | ... = spread operator constructs an array or sets the function arguments on invocation from an array | "category" is an array of categories in the database
    ...
    function addCategory() {
        axios.post("https://localhost:7165/categories", categoryUpdate)
            .then(response => {
    ...
    function deleteCategory() {
        axios.delete("https://localhost:7165/categories", { "params": { id: selectedCategory.id } })
            .then(response => {
        ...
        <Modal.Body className="text-center">
            ...
            {selectedCategory?.products?.map((element) => {
                return <Card className="my-2 mx-auto w-50">
                    <Card.Title>{element.name}{": "}<br></br>{element.description}<br></br>{"$"}{element.price}</Card.Title>
                </Card>
            })}
        </Modal.Body>
